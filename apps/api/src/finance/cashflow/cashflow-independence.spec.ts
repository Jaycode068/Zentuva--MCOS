...
describe('Cashflow Management independence (Sprint 15)', () => {
  ...
  const FORBIDDEN_WRITE_PATTERN =
    /\.(invoice|supplierInvoice|payment|supplierPayment|journalEntry|journalEntryLine|cashTransaction|bankStatementTransaction|bankStatementImport|bankReconciliation|reconciliationMatch|cashAccount|salesOrder|purchaseOrder|productionOrder|inventoryStock|inventoryTransaction)\.(create|update|updateMany|delete|deleteMany|upsert)\(/;

  it('structural guard: no cashflow file writes Invoice/SupplierInvoice/Payment/SupplierPayment/JournalEntry/CashAccount/CashTransaction/BankReconciliation, or any Sales/Procurement/Production/Inventory table, directly', () => {
