...
import { createSupplierPayment, type PaymentMethod, type SupplierInvoice } from '../api';
...
/**
 * "Record Payment" dialog for a Supplier Invoice (Sprint 12, docs/domains/finance.md
 * "Accounts Payable") — direct structural mirror of `invoices/payment-dialog.tsx`. The
 * server bounds an over-payment against `recognizedAmount` (never `total`) — the
 * client-side disabled-state below is a UX convenience only.
 */
        ...
        <div className="grid grid-cols-3 gap-x-4 gap-y-1 rounded-md bg-muted/30 p-3 text-xs text-muted-foreground">
          <div>
            ...
            <div className="font-medium text-foreground">
              {formatCurrency(invoice.recognizedAmount, invoice.currency)}
            </div>
          </div>
          <div>
            Paid
